...
So that subsequent PBIs can focus on application logic without setup overhead

## Preconditions

- Python 3.12 installed
- uv installed
- Git repo initialized on `feat/conversation-engine` branch (done)

## Context References — READ BEFORE IMPLEMENTING

- `CLAUDE.md` — All coding conventions, documentation requirements, naming rules
- `.agents/DevPlan.md` (lines 139-160) — Resolved decisions on tech stack, models, storage
...
- **IMPLEMENT**: Create `pyproject.toml` with the following structure:
...
- **GOTCHA**: `pydantic-settings` is a separate package from `pydantic` since v2. It must be listed explicitly. If using `uv add` instead of manual creation, run: `uv add pydantic-settings`.
...
- **GOTCHA**: `BaseSettings` comes from `pydantic_settings`, not `pydantic`.
...
| `src/config.py` | Settings loaded from env vars |
...
# Ruff runs (may have no files to check yet beyond config.py)
